/*
 * Schweizer ÖV Abfahrtsanzeiger für ESP32 mit OLED Display
 * Für LilyGO T3 1.6.1
 * 
...
// OLED Display - für LilyGO T3 mit 0.96" SSD1306
// Falls du ein 1.3" Display hast, ändere zu: U8G2_SH1106_128X64_NONAME_F_HW_I2C
...
const unsigned long DISPLAY_UPDATE_INTERVAL = 100;  // 100ms für flüssige Anzeige
...
// Hilfsfunktion: Ersetze Umlaute für Display
...
void setup() {
  ...
  Wire.begin(21, 22);  // SDA, SCL für LilyGO T3
  u8g2.begin();
  u8g2.enableUTF8Print();  // UTF-8 Unterstützung für Umlaute
...
void loop() {
  ...
  if (configMode) {
    ...
    // Update Display im Config-Modus
...
void displayBootScreen() {
  ...
  u8g2.drawStr(10, 25, "OeV Anzeige");  // Ö manuell ersetzt
  u8g2.setFont(u8g2_font_6x10_tr);
  u8g2.drawStr(20, 45, "LilyGO T3");
  u8g2.sendBuffer();
}
...
void displayStatus(const char* line1, const char* line2) {
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x10_tr);
  String l1 = replaceUmlauts(String(line1));
  String l2 = replaceUmlauts(String(line2));
  u8g2.drawStr(0, 15, l1.c_str());
  u8g2.drawStr(0, 35, l2.c_str());
  u8g2.sendBuffer();
}
...
void displayConfigMode() {
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x10_tr);
  u8g2.drawStr(0, 10, "CONFIG-MODUS");
  u8g2.drawLine(0, 12, 128, 12);
  
  u8g2.drawStr(0, 25, "SSID:");
  u8g2.drawStr(0, 35, "OEV-Anzeiger-Config");
  
  u8g2.drawStr(0, 48, "Pass: config123");
...
void displayDepartures() {
  ...
  // Station rechts von der Uhrzeit (gekürzt)
  String header = replaceUmlauts(stationName);
  if (header.length() > 14) {
    header = header.substring(0, 11);
    header += "...";
  }
  u8g2.drawStr(35, 7, header.c_str());
  u8g2.drawLine(0, 9, 128, 9);
  
  // Abfahrten anzeigen (max 4, kompakter)
  int y = 18;
  for (size_t i = 0; i < min((size_t)4, currentDepartures.size()); i++) {
    Departure& dep = currentDepartures[i];
    
    // Linie (max 4 Zeichen)
    String line = dep.line;
    if (line.length() > 4) line = line.substring(0, 4);
    u8g2.drawStr(0, y, line.c_str());
    
    // Ziel (gekürzt auf 9 Zeichen)
    String dest = replaceUmlauts(dep.destination);
    if (dest.length() > 9) {
      dest = dest.substring(0, 9);
      dest += ".";
    }
    u8g2.drawStr(28, y, dest.c_str());
    
    // Zeit
    u8g2.drawStr(88, y, dep.departureTime.c_str());
    
    // Verspätung
    if (dep.delay != 0) {
      String delayStr = String(dep.delay > 0 ? "+" : "") + String(dep.delay);
      u8g2.drawStr(115, y, delayStr.c_str());
    }
    
    y += 11;  // Kompakter Abstand (statt 12)
  }
  
  // Nächstes Update
  int secToUpdate = (UPDATE_INTERVAL - (millis() - lastUpdate)) / 1000;
  if (secToUpdate > 0) {
    String updateStr = String(">") + String(secToUpdate) + String("s");
    u8g2.drawStr(0, 63, updateStr.c_str());
  }
  ...
  // WiFi Icon (einfache Balken)
  for (int i = 0; i < bars; i++) {
    int h = 2 + i * 2;
    u8g2.drawBox(118 + i * 3, 63 - h, 2, h);
  }
...
void fetchAndDisplayDepartures() {
  ...
  HTTPClient http;
  String url = "http://transport.opendata.ch/v1/stationboard?station=" + urlEncode(stationName) + "&limit=4";
  ...
  if (httpCode == 200) {
    ...
    // Buffer für JSON (32KB für 4 Abfahrten)
    ...
    for (JsonObject connection : stationboard) {
      ...
      if (showThis && currentDepartures.size() < 4) {
        Departure dep;
