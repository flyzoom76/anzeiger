...
void handleDestinations() {
  ...
  HTTPClient http;
  // Limit auf 30 erhöht für mehr Verbindungsoptionen
...
void fetchDeparturesForStation(String station, String allowedDests, int maxDepartures, int walkingTime) {
  ...
  HTTPClient http;
  String url = "http://transport.opendata.ch/v1/stationboard?station=" + urlEncode(station) + "&limit=30";  // Limit auf 30 erhöht
  ...
  if (httpCode == 200) {
    ...
    // Payload ist ca. 300KB mit limit=30
    String payload = http.getString();
